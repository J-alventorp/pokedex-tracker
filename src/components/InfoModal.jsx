...
export default function InfoModal({ data, onClose }) {
  ...
  if (!data) return null;
  const { entity, card, selectedCardIds, onToggleCardChoice } = data;
            ...
            <>
              ...
              <div className="pc-modal-body">
                <p className="pc-modal-hint">
                  {onToggleCardChoice ? `Tap every card you've found of ${entity.name}:` : `Known printings of ${entity.name}:`}
                </p>
                ...
                {related.map((c) => {
                  const selected = !!onToggleCardChoice && selectedCardIds.includes(c.id);
                  return (
                    <div
                      className={`pc-modal-row ${onToggleCardChoice ? "selectable" : ""} ${selected ? "selected" : ""}`}
                      key={c.id}
                      onClick={onToggleCardChoice ? () => onToggleCardChoice(c.id) : undefined}
                    >
